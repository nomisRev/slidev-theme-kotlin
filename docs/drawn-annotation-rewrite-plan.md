...
| Theme-owned chrome | The code-window identity marks — icon and JDBC/R2DBC text badges, all `::after` pseudo-elements on the code wrapper, invisible to DOM collectors — and the copy control are theme-owned reserved boxes computed by the measurement adapter (pseudo-element geometry read via `getComputedStyle(el, '::after')`). `avoid-selector`/`reserve-selector` remain for genuinely custom consumer chrome only. |
...
4. ~~resolve materially useful no-verdict states~~ — done: `exposed-fundamentals/008-05`, `kotlin-fundamentals/060-02`, and the ambiguous `059-01` are resolved by the reviewer; the per-annotation details live in the analysis report's post-review clarifications;
